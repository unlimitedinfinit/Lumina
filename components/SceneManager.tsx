...
// Custom component for the glowing wormhole path
const CosmicPath: React.FC<{ points: Vector3[], theme: ThemeConfig, style: string }> = ({ points, theme, style }) => {
  const curve = useMemo(() => {
  ...
  const uniforms = useMemo(() => ({
    ...
    uStyle: { value: 0 } // 0: Default, 1: Neon, 2: Rainbow, 3: Dark
  }), [theme]);
  ...
  useFrame((state) => {
    if (shaderRef.current) {
      ...
      if (style === 'neon') shaderRef.current.uniforms.uStyle.value = 1;
      else if (style === 'rainbow') shaderRef.current.uniforms.uStyle.value = 2;
      else if (style === 'dark') shaderRef.current.uniforms.uStyle.value = 3;
      else shaderRef.current.uniforms.uStyle.value = 0;
    }
  });
  ...
  const fragmentShader = `
        ...
        void main() {
            ...
            if (uStyle < 0.5) { // DEFAULT (Thin, Clean)
                 core = pow(core, 4.0);
                 alpha = core * 0.4; // Low opacity
                 finalColor = mix(uColor, vec3(1.0), 0.5); // Desature slightly
            } 
            else if (uStyle < 1.5) { // NEON (Original)
                 core = pow(core, 3.0);
                 alpha = core * (0.5 + flow * 0.5);
                 finalColor = uColor + vec3(flow * 0.5);
            }
            else if (uStyle < 2.5) { // RAINBOW
                 core = pow(core, 3.0);
                 vec3 rain = hue2rgb(vUv.x * 0.2 + uTime * 0.5);
                 finalColor = rain + vec3(flow * 0.5);
                 alpha = core * 0.8;
            }
            else { // DARK
                 core = smoothstep(0.0, 0.2, core);
                 finalColor = vec3(0.0);
                 alpha = core * 0.9;
            }
  ...
  const width = style === 'default' ? 0.05 : (style === 'dark' ? 0.08 : 0.15); // DEFAULT IS THINNER
  ...
  return (
    <mesh>
      ...
      <shaderMaterial
        ...
        transparent
        depthWrite={false}
        blending={style === 'dark' ? 0 : AdditiveBlending} // No additive for dark
        side={DoubleSide}
      />
    </mesh>
  );
      ...
      {paths.map((path, index) => (
        path.length > 1 && (
          <CosmicPath
            ...
            points={path}
            theme={theme}
            style={sandboxSettings.trailStyle}
          />
        )
      ))}
