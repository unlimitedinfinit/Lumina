...
import { ARENA_BOUNDS, PARTICLE_COUNT, TARGET_FILL_RATE, GRAVITY_STRENGTH, PATH_FLOW_FORCE, FRICTION, MAX_SPEED, EMITTER_RATE, RANDOM_FORCE, CRITICAL_MASS_THRESHOLD, GRID_SNAP_RADIUS, CRYSTAL_VELOCITY_THRESHOLD, getObstaclePos } from '../constants';
...
const Swarm: React.FC<SwarmProps> = ({
    ...
    useFrame((state, delta) => {
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
            ...
            // BALANCED AGING: Lifespan doubled (approx 100s)
            ages[i] += dt * 0.01;
            if (ages[i] > 1.0) {
                life[i] = 0;
                dummy.position.set(-9999, -9999, 0);
            ...
            for (const anomaly of activeAnomalies) {
                ...
                if (distSq < anomaly.radius * anomaly.radius) {
                    if (anomaly.type === 'repulsor' || anomaly.type === 'pulse') {
                        ...
                        if (anomaly.type === 'hazard' || anomaly.type === 'comet') {
                            deathTimer[i] = 1.0;
                            break;
                        }
                        ...
                        if (anomaly.type === 'warp') {
                            vx *= 0.6; // Heavy drag (Time Freeze)
                            vy *= 0.6;
                        }
                        if (anomaly.type === 'prism') {
                            // Refract + Boost
                            const rot = 0.4; // Stronger bend
                            const nvx = vx * Math.cos(rot) - vy * Math.sin(rot);
                            const nvy = vx * Math.sin(rot) + vy * Math.cos(rot);
                            vx = nvx * 1.08; // Stronger acceleration
                            vy = nvy * 1.08;
                        }
                        if (anomaly.type === 'glitch') {
                            if (Math.random() < 0.2) { // 20% Chance (More Chaos)
                                x += (Math.random() - 0.5) * 4.0; // Wider Teleport
                                y += (Math.random() - 0.5) * 4.0;
                                positions[idx] = x; // Update buffer immediately
                                positions[idx + 1] = y;
                            }
                        }
                        // SPACE EVENTS
                        ...
                        if (anomaly.type === 'gamma') {
                            // Shockwave (Strong Repulsion)
                            const dist = Math.sqrt(distSq);
                            const force = (1 - dist / anomaly.radius) * 0.8; // Very strong
                            vx += (dx / dist) * force;
                            vy += (dy / dist) * force;
                            // Scatter
                        ...
                        if (anomaly.type === 'quasar') {
                            // Beam Rotation Effect (Simplified as strong tangential flow)
                            const dist = Math.sqrt(distSq);
                            // Push OUT and SPIN
                            const force = (1 - dist / anomaly.radius) * 0.2;
                            vx += (dx / dist) * force; // Out
                            vy += (dy / dist) * force;
            ...
            // --- CRYSTALLINE LATTICE (The Pyramid Concept) ---
            const crystalSpeedSq = vx * vx + vy * vy;
            // Only crystallize if old and slow
            if (age > 0.85 && crystalSpeedSq < CRYSTAL_VELOCITY_THRESHOLD * CRYSTAL_VELOCITY_THRESHOLD) {
                // Snap to grid
                const snapX = Math.round(x / GRID_SNAP_RADIUS) * GRID_SNAP_RADIUS;
                const snapY = Math.round(y / GRID_SNAP_RADIUS) * GRID_SNAP_RADIUS;

                const snapDx = snapX - x;
                const snapDy = snapY - y;

                // Spring force to grid point
                vx += snapDx * 0.1;
                vy += snapDy * 0.1;

                // Damping to freeze it in place
                vx *= 0.85;
                vy *= 0.85;

                // Visual Indicator (Logic handled in render loop via speed check)
            } else {
            ...
            if (trait !== TRAIT_GHOST) {
                // NEIGHBOR INTERACTIONS (Solar Collapse & Weavers)
                for (let k = 0; k < 12; k++) {
                    const otherPIdx = Math.floor(Math.random() * PARTICLE_COUNT);
                    const otherIdx = otherPIdx * 3;
                    ...
                    if (odistSq < 4.0 && odistSq > 0.0001) { // 2.0 Radius
                        const odist = Math.sqrt(odistSq);
                        const otherTrait = traits[otherPIdx];
                        const otherAge = ages[otherPIdx];

                        // 1. SOLAR COLLAPSE (Black Holes)
                        if (otherTrait === TRAIT_ROGUE && otherAge > 0.9) {
                            if (otherAge > 0.98) {
                                // SUPERNOVA BLAST (Push Away)
                                const blastForce = (2.0 - odist) * 2.5;
                                vx += (odx / odist) * blastForce;
                                vy += (ody / odist) * blastForce;
                            } else {
                                // GRAVITY WELL (Pull In)
                                const pullForce = (2.0 - odist) * 0.15;
                                vx -= (odx / odist) * pullForce;
                                vy -= (ody / odist) * pullForce;
                            }
                        }

                        // 2. WEAVER CONSTELLATIONS
                        if (trait === TRAIT_WEAVER && otherTrait === TRAIT_WEAVER) {
                            // Spring Force (Target Dist 0.8)
                            const targetDist = 0.8;
                            const springForce = (odist - targetDist) * 0.05; // Pull if far, Push if close
                            vx -= (odx / odist) * springForce;
                            vy -= (ody / odist) * springForce;
                        }

                        // Standard Repulsion (Anti-Clump)
                        if (odistSq < 0.16) {
                            const push = (0.4 - odist) * 0.8;
                            vx += (odx / odist) * push;
                            vy += (ody / odist) * push;
                        }
                    }
            ...
            for (let s = 0; s < subSteps; s++) {
                ...
                if (trait !== TRAIT_GHOST && levelConfig.obstaclePos) {
                    for (let o = 0; o < levelConfig.obstaclePos.length; o++) {
                        ...
                        let visualRadius = rad * 0.25;
                        if (type === 'blackhole') visualRadius = rad * 0.5;
                        if (type === 'cube' || type === 'pyramid') visualRadius = rad * 0.45; // Slightly larger hitbox
                        ...
                        } else if (type === 'static' || type === 'pulsar' || type === 'debris' || type === 'cube' || type === 'pyramid') {
                            if (distSq < collisionRad * collisionRad) {
                                const dist = Math.sqrt(distSq);
            ...
            // --- VISUAL OVERRIDES FOR EVOLUTION ---

            // 1. CRYSTAL LATTICE
            if (ages[i] > 0.85 && speedSq < CRYSTAL_VELOCITY_THRESHOLD * CRYSTAL_VELOCITY_THRESHOLD) {
                sx = 0.6 * scaleMult * GLOBAL_SCALE_MODIFIER;
                sy = 0.6 * scaleMult * GLOBAL_SCALE_MODIFIER;
                dummy.rotation.z = Math.PI / 4; // Diamond shape
            }

            // 2. SOLAR COLLAPSE (Black Hole / Supernova)
            if (trait === TRAIT_ROGUE && ages[i] > 0.9) {
                if (ages[i] > 0.98) {
                    // Supernova
                    sx = 6.0 * scaleMult;
                    sy = 6.0 * scaleMult;
                    dummy.rotation.z += Math.random(); // Spin fast
                } else {
                    // Black Hole Inter-phase
                    const pulse = 1 + Math.sin(state.clock.elapsedTime * 20) * 0.2;
                    sx = 0.7 * pulse; // Reduced from 1.0 (Round 2)
                    sy = 0.7 * pulse;
                }
            }
            ...
            if (sandboxSettings.rainbowMode) {
                ...
                // EVOLUTION COLORS
                if (trait === TRAIT_ROGUE && ages[i] > 0.9) {
                    if (ages[i] > 0.98) tempColor.setRGB(1, 1, 1); // Supernova White
                    else tempColor.setRGB(0.1, 0, 0.2); // Black Hole Purple
                } else if (ages[i] > 0.85 && speedSq < CRYSTAL_VELOCITY_THRESHOLD * CRYSTAL_VELOCITY_THRESHOLD) {
                    tempColor.setRGB(0, 1, 1); // Crystal Cyan
                    tempColor.multiplyScalar(2.0); // Glow
                } else if (trait === TRAIT_WEAVER) {
                    tempColor.setRGB(0.2, 0.6, 1.0); // Weaver Blue
                } else {
                    const ageNorm = Math.min(1, ages[i]);
                    // ... existing color logic ...
                    if (trait === TRAIT_GHOST) {
                        tempColor.setRGB(0.8, 1.0, 1.0);
                    } else if (trait === TRAIT_WEAVER) { // (Redundant but handled above now)
