...
const HybridEmitter: React.FC<{ position: [number, number, number], theme: ThemeConfig }> = ({ position, theme }) => {
    const groupRef = useRef<Group>(null);
    const ring1Ref = useRef<Mesh>(null);
    const ring2Ref = useRef<Mesh>(null);
    const crystalRef = useRef<Mesh>(null);
    const shardsRef = useRef<Group>(null);
    const riftRef = useRef<Mesh>(null);
    ...
    useFrame((state) => {
        ...
        if (ring2Ref.current) {
            ring2Ref.current.rotation.x = t * 0.6 + Math.PI / 2;
        ...
        if (crystalRef.current) {
            ...
            crystalRef.current.rotation.z = Math.sin(t * 0.5) * 0.1;
            crystalRef.current.position.y = Math.sin(t * 1.5) * 0.2; // Float
        }

        // Shards Orbit
        if (shardsRef.current) {
            shardsRef.current.rotation.y = -t * 0.5;
            shardsRef.current.rotation.z = Math.sin(t * 0.2) * 0.2;
        }

        // Rift Pulse
        if (riftRef.current) {
            riftRef.current.rotation.z = t * 0.2;
            const uniqueScale = 1 + Math.sin(t * 3) * 0.05;
            riftRef.current.scale.set(uniqueScale, uniqueScale, uniqueScale);
        }
    });
    ...
    return (
        <group position={new Vector3(...position)} ref={groupRef}>
            {/* Base Rift (Celestial) */}
            <mesh ref={riftRef} rotation={[-Math.PI / 2, 0, 0]} position={[0, -1.5, 0]}>
            ...
            {/* Quantum Rings (Sci-Fi) */}
            <group>
                <mesh ref={ring1Ref}>
                    <torusGeometry args={[1.2, 0.05, 16, 64]} />
                    <meshStandardMaterial color="#88aaff" metalness={1.0} roughness={0.1} />
                </mesh>
            ...
            {/* Inner Light */}
            <pointLight color={theme.colors.primary} intensity={2} distance={5} />

            {/* Floating Shards (Fantasy) */}
            <group ref={shardsRef}>
                {[0, 1, 2].map(i => {
                    const angle = (i / 3) * Math.PI * 2;
                    return (
                        <mesh key={i} position={[Math.cos(angle) * 1.8, Math.sin(i * 10) * 0.2, Math.sin(angle) * 1.8]} rotation={[Math.random(), Math.random(), 0]}>
                            <tetrahedronGeometry args={[0.2, 0]} />
                            <meshStandardMaterial color={theme.colors.secondary} emissive={theme.colors.secondary} emissiveIntensity={1} />
                        </mesh>
                    );
                })}
            </group>
...
const HybridTarget: React.FC<{
    position: [number, number, number],
    radius: number,
    theme: ThemeConfig,
    completion: number
}> = ({ position, radius, theme, completion }) => {
    const groupRef = useRef<Group>(null);
    const ring1Ref = useRef<Mesh>(null);
    const ring2Ref = useRef<Mesh>(null);
    const coreRef = useRef<Mesh>(null);
    ...
    useFrame((state) => {
        ...
        // Dynamic Intensity based on completion
        const safeCompletion = Math.min(1, Math.max(0, completion));
        const intense = 1 + safeCompletion * 2.0;

        // Rings: Tech spin
        if (ring1Ref.current) {
            ring1Ref.current.rotation.x = t * 0.4 * intense;
            ring1Ref.current.rotation.y = t * 0.2 * intense;
        }
        if (ring2Ref.current) {
            ring2Ref.current.rotation.x = Math.PI / 2 + t * 0.3 * intense;
            ring2Ref.current.rotation.z = -t * 0.5 * intense;
        }

        // Core: Pulsating Heart
        if (coreRef.current) {
            const beat = 1 + Math.sin(t * (3 + safeCompletion * 5)) * (0.1 + safeCompletion * 0.2);
            const size = (radius * 0.4) * beat * (1 + safeCompletion);
            coreRef.current.scale.set(size, size, size);
            coreRef.current.rotation.y += 0.02 * intense;
            coreRef.current.rotation.z += 0.03 * intense;
        }

        // Vortex: Suction Effect
        if (vortexRef.current) {
            vortexRef.current.rotation.z = -t * (2.0 + safeCompletion * 4.0);
            vortexRef.current.scale.setScalar(1.0 + Math.sin(t * 10) * 0.05);
        }
    });

    return (
        <group position={new Vector3(...position)} ref={groupRef}>
            {/* Vortex Rings (Suction) */}
            <group ref={vortexRef}>
                {[0.8, 1.2, 1.6].map((scale, i) => (
                    <mesh key={i} rotation={[0, 0, (i * Math.PI) / 3]}>
                        <torusGeometry args={[radius * scale, 0.02 * scale, 8, 3]} />
                        <meshBasicMaterial color={theme.colors.primary} transparent opacity={0.3 - i * 0.05} />
                    </mesh>
                ))}
            </group>

            {/* Tech Rings (Containment) */}
            <mesh ref={ring1Ref}>
                <torusGeometry args={[radius * 1.8, 0.05, 8, 32]} />
                <meshStandardMaterial color="#444444" emissive={theme.colors.primary} emissiveIntensity={0.5} roughness={0.2} metalness={0.8} />
            </mesh>
            <mesh ref={ring2Ref}>
                <torusGeometry args={[radius * 1.5, 0.08, 8, 32]} />
                <meshStandardMaterial color="#222222" emissive={theme.colors.secondary} emissiveIntensity={0.3} roughness={0.2} metalness={0.9} />
            </mesh>
            ...
            <mesh ref={coreRef}>
                ...
                <meshStandardMaterial
                    color="#000000"
                    emissive={theme.colors.primary}
                    emissiveIntensity={1 + completion * 2}
                    roughness={0.0}
                    metalness={1.0}
                />
            </mesh>
            ...
            <pointLight color={theme.colors.primary} intensity={2 + completion * 2} distance={8} />
        </group>
    );
};
...
const LevelElements: React.FC<LevelElementsProps> = ({ config, sandboxSettings, theme, completionRatio, blackHoleStateRef }) => {
            ...
            obstaclesRef.current.children.forEach((child, i) => {
                ...
                if (type === 'blackhole') {
                    ...
                    if (blackHoleStateRef && blackHoleStateRef.current && blackHoleStateRef.current[i] !== undefined) {
                        ...
                        // REDUCED GROWTH for Playability (User Requested 75% reduction)
                        const growthFactor = 1.0 + (mass / 50.0) * 1.5;
                        const baseScale = 0.4 * growthFactor;
    ...
    return (
        <>
            ...
            {/* Hybrid Emitter (Crystal + Quantum + Celestial) */}
            <HybridEmitter position={config.emitterPos} theme={theme} />
            ...
            {config.obstaclePos && config.obstaclePos.length > 0 && (
                <group ref={obstaclesRef}>
                    {config.obstaclePos.map((pos, idx) => {
                        ...
                        if (type === 'blackhole') {
                            const visualScale = 0.4; // Reduced from 0.6 (Round 2)
                            return (
                                <group key={`obs-${idx}`}>
                                    {/* Event Horizon */}
                                    <mesh>
                        ...
                        if (type === 'cube') {
                            return (
                                <group key={`obs-${idx}`}>
                                    <mesh>
                                        <boxGeometry args={[radius * 0.5, radius * 0.5, radius * 0.5]} />
                                        <meshStandardMaterial color={theme.colors.secondary} metalness={0.8} roughness={0.2} />
                                    </mesh>
                                    <mesh scale={[1.1, 1.1, 1.1]}>
                                        <boxGeometry args={[radius * 0.5, radius * 0.5, radius * 0.5]} />
                                        <meshBasicMaterial color={theme.colors.secondary} wireframe transparent opacity={0.5} />
                                    </mesh>
                                </group>
                            );
                        }
                        ...
                        if (type === 'pyramid') {
                            return (
                                <group key={`obs-${idx}`}>
                                    <mesh>
                                        <tetrahedronGeometry args={[radius * 0.6, 0]} />
                                        <meshStandardMaterial color={theme.colors.primary} metalness={0.9} roughness={0.1} />
                                    </mesh>
